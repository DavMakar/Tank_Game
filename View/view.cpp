...
View::View(Game & game_model):game_model_{game_model},menu_view_{game_model.menu_model_}
{
    initscr();
    use_default_colors();
    noecho();
...
void View::render(){
    ...
    for(int i = 1 ; i<game_matrix.size()-1;++i){
        for(int j = 1;j< game_matrix[0].size()-1;++j){
            ...
            {
            case Map::Map_title::wall:
                draw_wall(i,j);
                break;
            case Map::Map_title::player:
                draw_player(i,j);
                break;
            case Map::Map_title::enemy:
                draw_enemy(i,j);
                break;
            case Map::Map_title::bullet:
                draw_bullet(i,j);
                break;
            case Map::Map_title::empty:
                draw_empty(i,j);
            default:
                break;
            }
